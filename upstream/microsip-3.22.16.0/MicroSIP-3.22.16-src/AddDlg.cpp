...
AddDlg::AddDlg(CWnd* pParent /*=NULL*/)
    : CThemeDialog(AddDlg::IDD, pParent)
{
    Create(IDD, pParent);
}
...
BEGIN_MESSAGE_MAP(AddDlg, CThemeDialog)
    ON_WM_CREATE()
    ON_WM_CLOSE()
    ...
    if (!contact.number.IsEmpty()) {
        if (number.IsEmpty() || number != contact.number) {
            if (mainDlg->pageContacts->FindContact(contact.number)) {
                if (MessageBox(Translate(_T("A contact with this phone number already exists. Do you want to replace it?")), Translate(_T("Contact already exists")), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) != IDYES) {
                    return;
                }
            }
        }
