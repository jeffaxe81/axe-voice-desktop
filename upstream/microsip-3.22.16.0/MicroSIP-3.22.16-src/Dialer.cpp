...
void Dialer::RebuildButtons(bool init)
{
    ...
    if (::IsWindow(m_ButtonDND.m_hWnd)) {
        m_ButtonDND.DestroyWindow();
    }
    if (::IsWindow(m_ButtonFWD.m_hWnd)) {
        m_ButtonFWD.DestroyWindow();
    }
    if (::IsWindow(m_ButtonAA.m_hWnd)) {
        m_ButtonAA.DestroyWindow();
    }
    if (::IsWindow(m_ButtonAC.m_hWnd)) {
        m_ButtonAC.DestroyWindow();
    }
    if (::IsWindow(m_ButtonConf.m_hWnd)) {
        m_ButtonConf.DestroyWindow();
    }
    if (::IsWindow(m_ButtonRec.m_hWnd)) {
    ...
    if (addDND || addFWD || addAC || addAA || addConf || addRec) {
        CRect windowRect;
        if (!init) {
...
#ifdef _GLOABL_BOTTOM_BUTTONS_CENTER
        int count = addDND + addFWD + addAC + addAA + addRec;
        int countWide = addConf;
        int blockWidth = count * stepPx + countWide * (stepPx + mapRect.top);
        int areaWidth = windowSize.x - mainDlg->widthAdd;
        int offsetRight = areaWidth - (areaWidth - blockWidth) / 2;
        int newLeft = offsetRight - rect.Width() - mapRect.bottom / 2;
        int newRight = offsetRight - mapRect.bottom / 2;
        if (!m_isButtonVoicemailVisible || newRight < rect.left) {
            rect.left = newLeft;
            rect.right = newRight;
        }
#else
...
#ifdef _GLOABL_BOTTOM_BUTTONS_CENTER
            AutoMove(m_ButtonRec.m_hWnd, 50, 100, 0, 0);
#else 
            AutoMove(m_ButtonRec.m_hWnd, 100, 100, 0, 0);
#endif
            m_ButtonRec.SetTooltip(Translate(_T("Call Recording")));
            rect.left -= stepPx;
            rect.right -= stepPx;
        }
        if (addConf) {
...
#ifdef _GLOABL_BOTTOM_BUTTONS_CENTER
            AutoMove(m_ButtonConf.m_hWnd, 50, 100, 0, 0);
#else 
            AutoMove(m_ButtonConf.m_hWnd, 100, 100, 0, 0);
#endif
            m_ButtonConf.SetTooltip(Translate(_T("Conference")));
            rect.left -= stepPx;
            rect.right -= stepPx;
        }
        if (addAA) {
...
#ifdef _GLOABL_BOTTOM_BUTTONS_CENTER
            AutoMove(m_ButtonAA.m_hWnd, 50, 100, 0, 0);
#else 
            AutoMove(m_ButtonAA.m_hWnd, 100, 100, 0, 0);
#endif
            m_ButtonAA.SetTooltip(Translate(_T("Auto Answer")));
            rect.left -= stepPx;
            rect.right -= stepPx;
        }
        if (addAC) {
...
#ifdef _GLOABL_BOTTOM_BUTTONS_CENTER
            AutoMove(m_ButtonAC.m_hWnd, 50, 100, 0, 0);
#else 
            AutoMove(m_ButtonAC.m_hWnd, 100, 100, 0, 0);
#endif
            m_ButtonAC.SetTooltip(Translate(_T("Auto Conference")));
            rect.left -= stepPx;
            rect.right -= stepPx;
        }
        if (addFWD) {
...
#ifdef _GLOABL_BOTTOM_BUTTONS_CENTER
            AutoMove(m_ButtonFWD.m_hWnd, 50, 100, 0, 0);
#else 
            AutoMove(m_ButtonFWD.m_hWnd, 100, 100, 0, 0);
#endif
            m_ButtonFWD.SetTooltip(Translate(_T("Call Forwarding")));
            rect.left -= stepPx;
            rect.right -= stepPx;
        }
        if (addDND) {
...
#ifdef _GLOABL_BOTTOM_BUTTONS_CENTER
            AutoMove(m_ButtonDND.m_hWnd, 50, 100, 0, 0);
#else 
            AutoMove(m_ButtonDND.m_hWnd, 100, 100, 0, 0);
#endif
            m_ButtonDND.SetTooltip(Translate(_T("Do Not Disturb")));
            rect.left -= stepPx;
            rect.right -= stepPx;
        }
        if (!init) {
...
void Dialer::RebuildShortcutsRestart()
{
    if (accountSettings.enableShortcuts || mainDlg->shortcutsEnabled) {
        if (mainDlg->shortcutsEnabled != accountSettings.enableShortcuts
            ...
            ||
            (!accountSettings.shortcutsBottom && (int)((shortcuts.GetCount() + _GLOBAL_SHORTCUTS_ROWS - 1) / _GLOBAL_SHORTCUTS_ROWS) != (int)((mainDlg->shortcutsCount + _GLOBAL_SHORTCUTS_ROWS - 1) / _GLOBAL_SHORTCUTS_ROWS))
        ...
        for (int i = 0; i < shortcuts.GetCount(); i++) {
            ...
            if (mainDlg->shortcutsBottom) {
                ...
                if (
                    ...
                    button->Create(Translate(shortcut.label), style, buttonRect, this, IDC_SHORTCUT_RANGE + i);
                    if (!row) {
    ...
    if (shortcuts.GetCount() == shortcutButtons.GetCount()) {
        ...
        for (int i = 0; i < shortcuts.GetCount(); i++) {
            ...
            if (shortcut->presence || fromUsersDirectory) {
                CString numberFormated = shortcut->number;
                if (!fromUsersDirectory) {
                    CString commands;
                    numberFormated = FormatNumber(numberFormated, &commands, true);
                }
...
BOOL Dialer::OnInitDialog()
{
    ...
    LPCTSTR p = Translate(_T("Voicemail Number"));
    m_ToolTip.AddTool(&m_ButtonVoicemail, p);
    m_ToolTip.AddTool(&m_ButtonVoicemailDisabled, p);
    ...
    //    m_SliderCtrlInput.IsDark = lastDark == 1;
    //    m_SliderCtrlOutput.IsDark = m_SliderCtrlInput.IsDark;
...
BEGIN_MESSAGE_MAP(Dialer, CBaseDialog)
    ...
    ON_COMMAND_RANGE(IDC_SHORTCUT_RANGE, IDC_SHORTCUT_RANGE + 99, &Dialer::OnBnClickedShortcut)
    ON_WM_RBUTTONUP()
    ON_WM_LBUTTONUP()
    ON_WM_MOUSEMOVE()
...
{
    CComboBox* combobox = (CComboBox*)GetDlgItem(IDC_NUMBER);
    CButton* button = (CButton*)GetDlgItem(IDC_DIALER_DTMF);
    CRect rect;
    ...
    if (!digits.IsEmpty()) {
        SetNumber(digits);
        if (!isButtonVisisble) {
            button->SetWindowPos(NULL, 0, 0, buttonRect.Width(), rect.Height(), SWP_NOZORDER | SWP_NOMOVE);
            combobox->SetWindowPos(NULL, 0, 0, rect.Width() - buttonRect.Width(), rect.Height(), SWP_NOZORDER | SWP_NOMOVE);
...
void Dialer::SetNumber(CString number, int callsCount)
{
    ...
    if (old.IsEmpty() || number.Find(old) != 0) {
        if (!number.IsEmpty()) {
            number.AppendChar(' '); // prevent combobox autocomplete
        }
...
void Dialer::Action(DialerActions action)
{
    ...
    if (!number.IsEmpty()) {
        ...
        if (action != ACTION_MESSAGE) {
            res = mainDlg->MakeCall(numberLocal, action == ACTION_VIDEO_CALL);
        }
        else {
            res = mainDlg->MessagesOpen(numberLocal);
        }
        if (res) {
...
void Dialer::OnBnClickedMuteOutput()
{
    ...
    button->SetCheck(button->GetCheck() == 0);
}
...
void Dialer::OnBnClickedShortcut(UINT nID)
{
    if (shortcuts.GetCount() == shortcutButtons.GetCount()) {
        ...
        Shortcut shortcut = shortcuts.GetAt(i);
        mainDlg->ShortcutAction(&shortcut, false, !(((CButton*)GetDlgItem(nID))->GetCheck() & BST_CHECKED));
        if (!(((CButton*)GetDlgItem(nID))->GetCheck() & BST_CHECKED)) {
            GetDlgItem(nID)->SetWindowText(shortcut.label);
        }
        else {
            if (!shortcut.label2.IsEmpty()) {
                GetDlgItem(nID)->SetWindowText(shortcut.label2);
            }
        }
    }
}
