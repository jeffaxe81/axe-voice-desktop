...
namespace {
	...
	int CALLBACK SortFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
	{
        ...
        if ((ps.m_ColumnIndex == 2 || ps.m_ColumnIndex == 3) && mainDlg && mainDlg->pageCalls) {
			...
			if (list && ps.m_hWnd==list->m_hWnd) {
                ...
                if (ps.m_ColumnIndex == 2) {
                    ...
                    if (ps.m_Ascending) {
                        if (pCall1->duration > pCall2->duration) return 1;
                        else if (pCall1->duration < pCall2->duration) return -1;
                        else return 0;
                    }
                    else {
                        if (pCall1->duration > pCall2->duration) return -1;
